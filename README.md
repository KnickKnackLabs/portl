...
**v0.10.7** — Herdr local-attach patch. Portl has persistent terminal
sessions via `portl/session/v1`, provider discovery, Herdr bridge
attach, zmx-control support, tmux `-CC` compatibility, `PORTL-S-*`
short codes for importing shared session access through `portl accept`,
and stable host-suffixed labels for paired machines and saved access.
...
`PORTL_VERSION=0.10.7` to pin a release. Use `--agent=off` to disable the
...
```bash
# mise
mise use -g github:KnickKnackLabs/portl@0.10.7
