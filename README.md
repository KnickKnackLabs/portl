...
**v0.6.1** — short online session-sharing release. Portl has persistent
terminal sessions via `portl/session/v1`, provider discovery,
zmx-control support, tmux `-CC` compatibility, and `PORTL-S-*` short
codes for importing shared session access through `portl accept`.
...
`PORTL_VERSION=0.6.1` to pin a release. Use `--agent=off` to disable the
...
```bash
# mise
mise use -g github:KnickKnackLabs/portl@0.6.1
