...
pub(crate) struct BufferedRecv {
    prefix: Vec<u8>,
    recv: RecvStream,
}

impl BufferedRecv {
    pub(crate) fn new(recv: RecvStream, prefix: Vec<u8>) -> Self {
        Self { prefix, recv }
    }
    ...
    pub(crate) fn push_front(&mut self, bytes: &[u8]) {
        let mut prefix = bytes.to_vec();
        prefix.extend_from_slice(&self.prefix);
        self.prefix = prefix;
    }

    pub(crate) async fn read_frame<T>(&mut self, max_bytes: usize) -> Result<Option<T>>
    where
        T: DeserializeOwned,
    {
        loop {
...
pub(crate) async fn read_postcard_prefix<T>(
    recv: RecvStream,
    max_bytes: usize,
