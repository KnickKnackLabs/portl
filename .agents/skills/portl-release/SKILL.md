...
5. Run `mise run release:verify -- VERSION --full` before commit/tag unless the user explicitly requests a lighter check. The `--full` path intentionally opts into ignored slow e2e smoke tests; do not tag a release until these pass locally.
...
- `mise run release:verify -- VERSION --full` — metadata checks, fmt/diff/version, focused tests, clippy, and ignored slow e2e smoke tests that regular CI skips. Add `--allow-existing-tag` only when auditing an already-tagged release.
